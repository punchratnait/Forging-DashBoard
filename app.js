...
function pageAllowedRoutes(){
  ...
  if(PAGE_VIEW === 'heattreatment') return ['dashboard']; // dedicated HT Dashboard URL — gated at login to supervisor/admin only
  ...
  <div class="panel">
    ...
    <div class="table-scroll">
      <table>
        <thead><tr><th></th>${periods.map(colHeader).join('')}</tr></thead>
        <tbody>
          <tr><td>Production (Kg)</td>${periods.map((p,i)=>`<td style="font-family:var(--mono);">${cellVal(i,'totalKg')}</td>`).join('')}</tr>
          <tr><td>ERP Done (Kg)</td>${periods.map((p,i)=>`<td style="font-family:var(--mono);">${cellVal(i,'erpDoneKg')}</td>`).join('')}</tr>
          <tr><td>ERP Pending (Kg)</td>${periods.map((p,i)=>{
            ...
            return `<td style="font-family:var(--mono); color:${c.pendingKg>0?'var(--amber)':'var(--green)'};">${fmt1(c.pendingKg)}</td>`;
          }).join('')}</tr>
...
function pageHtDashboardContent(){
  ...
  <div class="panel" style="margin-top:18px;">
    ...
    <div class="grid-4">
      ...
      <div class="kpi-card"><div class="kpi-label">Total Production (Kg)</div><div class="kpi-value" style="font-size:20px;">${fmt1(totalKgMtd)}</div></div>
...
function pageHtEntryLog(){
  ...
  <div class="panel">
    ...
    <div style="font-size:12.5px; color:var(--ink-dim); margin-bottom:14px;">
      <b style="color:var(--amber);">${rows.length}</b> entr${rows.length===1?'y':'ies'} found${rows.length>0 ? ` · <b style="color:var(--ink);">${fmt1(rows.filter(e=>(e.qtyUnit||'kg')!=='pcs').reduce((s,e)=>s+(Number(e.qtyKg)||0),0))} kg</b>${rows.some(e=>e.qtyUnit==='pcs') ? ` + <b style="color:var(--ink);">${rows.filter(e=>e.qtyUnit==='pcs').reduce((s,e)=>s+(Number(e.qtyKg)||0),0)} pcs</b>` : ''} total` : ''}
    </div>
    <div class="table-scroll">
      <table>
        <thead><tr>
          ...
          ${rows.length===0 ? `<tr class="empty-row"><td colspan="17">No HT entries found.</td></tr>` :
            rows.map(e=>{
              ...
              <tr>
                ...
                <td style="font-family:var(--mono);">${fmt1(e.qtyKg)} ${e.qtyUnit==='pcs'?'pcs':'kg'}</td>
                <td style="font-family:var(--mono);">${fmt1(e.erpDoneKg)} ${e.qtyUnit==='pcs'?'pcs':'kg'}</td>
                <td style="font-family:var(--mono); color:${pendingKg>0?'var(--amber)':'var(--green)'};">${fmt1(pendingKg)} ${e.qtyUnit==='pcs'?'pcs':'kg'}</td>
