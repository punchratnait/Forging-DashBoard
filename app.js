...
function pageHtDashboardContent(){
  ...
  const mtdHt = scoped.filter(e=>inRange(e.date, monthStartStr(), todayStr()));
  const totalKgMtd = mtdHt.filter(e=>(e.qtyUnit||'kg')!=='pcs').reduce((s,e)=>s+(Number(e.qtyKg)||0), 0);
  const totalPcsMtd = mtdHt.filter(e=>e.qtyUnit==='pcs').reduce((s,e)=>s+(Number(e.qtyKg)||0), 0);
  const dtMinMtd = mtdHt.reduce((s,e)=>s+(e.downtimeBlocks||[]).reduce((s2,b)=>s2+(Number(b.minutes)||0),0), 0);
  const jobChangeCountMtd = mtdHt.reduce((s,e)=>s+(Number(e.jobChangeCount)||0), 0);
  const jobChangeLossMtd = mtdHt.reduce((s,e)=>s+(Number(e.jobChangeLossMinutes)||0), 0);
  const furnaceTonnage = htTonnageByFurnace(monthStartStr(), todayStr());
  ...
  <div class="panel" style="margin-top:18px;">
    <div class="panel-title"><span class="bar"></span>Furnace-wise Production (Month to Date)</div>
    <div class="table-scroll">
      <table>
  ...
  <div class="panel" style="margin-top:18px;">
    <div class="panel-title"><span class="bar"></span>Down Time &amp; Job Change (Month to Date)</div>
    <div class="grid-4">
