...
function renderShell(){
  ...
  const viewLabel = PAGE_VIEW==='entry' ? 'ENTRY TERMINAL' : PAGE_VIEW==='dashboard' ? 'MANAGEMENT VIEW' : PAGE_VIEW==='l1' ? 'EXECUTIVE VIEW' : PAGE_VIEW==='supervisor' ? 'SUPERVISOR VIEW' : 'ADMIN CONSOLE';
  const labels = {dashboard:'Dashboard', newentry:'New Entry', entrylog:'Entry Log', masterdata:'Master Data', reports:'Reports', spm:'SPM', l1:'Executive Dashboard', htnewentry:'HT New Entry', htentrylog:'HT Entry Log'};
...
function renderPage(){
  ...
  if(ROUTE==='dashboard') return SESSION.role==='supervisor' ? pageSupervisorDashboard() : pageDashboard();
...
async function boot(){
  ...
  try{
    ...
    if(savedId){
      ...
      const blocked = user && ((PAGE_VIEW === 'dashboard' && !userHasDashboard) || (PAGE_VIEW === 'l1' && !userHasL1) || (PAGE_VIEW === 'supervisor' && !userHasSupervisorPage));
      if(user && !blocked){
