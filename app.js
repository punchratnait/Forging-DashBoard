...
function pageAllowedRoutes(){
  ...
  if(PAGE_VIEW === 'entry') return canSpm ? ['newentry','entrylog','spm'] : ['newentry','entrylog'];
  ...
  if(role === 'supervisor') return ['dashboard','newentry','entrylog','spm'];
  return ['dashboard','newentry','entrylog']; // operator
}
...
function buildMachineSummary(entries){
  ...
  return Object.keys(byMachine).map(mid=>{
    ...
    return { mid, plannedQty:Math.round(plannedQty), actualQty, weightKg:fmt1(a.totalWeightKg), oeePct:a.oeePct, downtimePct:a.downtimePct, lossText, lossHtml };
...
function attachShellEvents(){
  document.getElementById('logoutBtn').onclick = ()=>{ if(inactivityTimer) clearTimeout(inactivityTimer); try{ sessionStorage.removeItem('prf_session_userid'); }catch(e){} SESSION=null; LOGIN_MODE='login'; render(); };
...
function renderExtraItemsHtml(items){
  ...
  return items.map(it=>{
    ...
    <div class="extra-item-row" data-itemrowid="${it.id}" style="display:flex; gap:10px; align-items:flex-end; margin-bottom:14px; flex-wrap:wrap; padding-bottom:10px; border-bottom:1px dashed var(--line);">
      <div class="field" style="flex:1; min-width:200px; margin-bottom:0;"><label>Item</label>
        <input type="text" class="ei-item-search" data-itemrowid="${it.id}" list="itemDatalist" autocomplete="off" placeholder="Search item code or name..." value="${label}">
      </div>
    ...
    <form id="entryForm">
      ...
      <div class="form-row">
        ...
        <div class="field"><label>Item</label>
          <input type="text" id="f_item_search" list="itemDatalist" autocomplete="off" placeholder="Search item code or name..." value="${d.itemId && byId(DB.items,d.itemId) ? itemLabel(byId(DB.items,d.itemId)) : ''}">
          <datalist id="itemDatalist">${DB.items.map(it=>`<option value="${itemLabel(it)}">`).join('')}</datalist>
          <select id="f_item" required style="display:none;">${renderItemOptions(DB.items, d.itemId, 'Select item')}</select>
          <div class="helptext">Search any item by code or name — not limited to items already mapped to this machine. SPM auto-fills if a rate exists for this Machine + Item.</div>
        </div>
      </div>
      <div class="form-row">
...
function attachNewEntryEvents(){
  ...
  const selectItem = ()=>{
    const typed = itemSearchInput.value.trim();
    const match = DB.items.find(it=>itemLabel(it)===typed);
    itemSel.value = match ? match.id : '';
    updateSpm(); updateItemInfo(); updateWeightFromQty(); recomputeSummary();
  };
  itemSearchInput.oninput = selectItem;
  itemSearchInput.onchange = selectItem;
...
function attachEditAccountModalEvents(userId){
  ...
  document.getElementById('editAccountForm').onsubmit = async (ev)=>{
    ...
    await save('users');
    closeModal();
    if(userId === SESSION.id){ SESSION.name = newName; }
    render();
...
async function boot(){
  ...
  try{
    ...
    if(savedId){
      ...
      if(user){
        SESSION = {id:user.id, name:user.name, role:user.role, locationId:user.locationId||''};
