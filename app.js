...
const STORE_KEYS = {
  ...
  roleLabels:'prf_role_labels'
};

let DB = { users:[], machines:[], items:[], spm:[], locations:[], sites:[], shifts:[], reasons:[], entries:[], invoices:[], plantmetrics:[], complaints:[], machineholds:[], htfurnaceitems:[], htreasons:[], htentries:[], roleLabels:[] };
...
const DEPARTMENT_ITEM_CODE_PATTERNS = {
  forging: ['FG'],
  heattreatment: ['HT','IH','SR']
};
...
function itemsForDepartment(dept){
  const patterns = DEPARTMENT_ITEM_CODE_PATTERNS[dept];
  if(!patterns || patterns.length===0) return DB.items; // no pattern configured for this department = show all
  return DB.items.filter(it=>{
    const code = (it.itemCode||'').toUpperCase();
    return patterns.some(p=>code.includes(p.toUpperCase()));
  });
}
...
function pageHtEntryLog(){
  ...
  if(htLogFilters.search && htLogFilters.search.trim()){
    ...
    rows = rows.filter(e=>{
      ...
      const item = byId(DB.items, e.itemId) || {};
      const haystack = [furnace.name, furnace.machineCode, item.name, item.itemCode, e.operatorName, e.supervisorName].join(' ').toLowerCase();
      return haystack.includes(q);
    });
  }
