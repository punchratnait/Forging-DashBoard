...
async function seedIfEmpty(){
  ...
  if(!DB.shifts.some(s=>shiftDepartment(s)==='heattreatment')){
    ...
    DB.shifts.push({id:uid('sh'),name:'Shift B',startTime:'19:00',durationMinutes:660,breakMinutes:30,department:'heattreatment'});
    changed = true;
  }
  ...
  <div class="panel">
    <form id="htEntryForm">
      <div class="form-row">
        ...
        <div class="field"><label>Shift</label><select id="ht_shift" required>${opts(shiftsForDepartment('heattreatment'), d.shiftId, 'Select shift')}</select></div>
      ...
      <div class="form-row">
        ...
        <div class="field"><label>Furnace</label><select id="ht_furnace" required>${opts(machinesForLocationDept(d.locationId,'heattreatment'), d.furnaceId, 'Select furnace')}</select>
      ...
      <div class="form-row">
        <div class="field"><label>Operator Name</label><input type="text" id="ht_operator" value="${d.operatorName}" readonly style="opacity:0.7;"></div>
        <div class="field"><label>Supervisor</label><select id="ht_supervisor" required>${opts(supervisorOptionsForCurrentUser(), d.supervisorId, 'Select supervisor')}</select></div>
      </div>
...
function attachHtNewEntryEvents(){
  ...
  locationSel.onchange = ()=>{
    furnaceSel.innerHTML = renderOptions(machinesForLocationDept(locationSel.value,'heattreatment'), '', 'Select furnace');
  };
  furnaceSel.onchange = ()=>{
    itemSearchInput.value=''; itemSel.value=''; itemSuggestBox.style.display='none'; itemSuggestBox.innerHTML='';
    updateItemDesc();
  ...
  if(addHtExtraItemBtn){
    addHtExtraItemBtn.onclick = ()=>{
      htEntryDraft.items.push({id: uid('htei'), itemId:'', qtyKg:0, erpDoneKg:0});
      extraItemsContainer.innerHTML = renderHtExtraItemsHtml(htEntryDraft.items);
      wireHtExtraItemTypeaheads();
    };
  }
    ...
    <div class="table-scroll">
      <table>
        <thead><tr>
          ...
          <th>Qty (Kg)</th><th>ERP Done</th><th>Pending</th><th>Job Chg</th><th>Job Loss (min)</th><th>D/T Min</th><th>Reason</th><th>Operator</th><th>Supervisor</th><th></th>
        </tr></thead>
        <tbody>
...
function attachHtEntryLogEvents(){
  ...
  if(exportBtn) exportBtn.onclick = ()=>{
    ...
    const header = ['Date','Shift','Site','Location','Furnace','Job Order No.','Product','Qty (Kg)','ERP Done','Pending','Job Changes','Job Loss (min)','Down Time (min)','Reason','Operator','Supervisor'];
