...
function machineStatusGrid(startDate, endDate){
  ...
  const perLocation = locsToShow.map(loc=>{
    const machinesInLoc = DB.machines.filter(m=>m.location===loc.id && machineDepartment(m)==='forging');
    const machines = machinesInLoc.map(m=>{
      const sharedIds = machineIdsSharingCode(m);
      const statusByDate = dates.map(dt=>{
        const entered = DB.entries.some(e=>e.date===dt && sharedIds.includes(e.machineId));
...
function renderMachineStatusSection(){
  const {start,end} = resolveStatusRange();
  const grid = machineStatusGrid(start,end);
  const dates = grid.dates;
  const rows = grid.perLocation;
  if(rows.length === 0) return `${machineStatusModeSelectorHtml()}<div class="helptext">No machines configured${effectiveLocationId()?' for your location':''}.</div>`;
