    ...
    <form id="entryForm">
      ...
      <div class="form-row">
        ...
        <div class="field"><label>Item</label><select id="f_item" required>${renderItemOptions(itemsForMachine(d.machineId), d.itemId, 'Select item')}</select>
          <div class="helptext">Filtered to items mapped (via SPM) to the selected machine. Shown as Item Code — Name.</div>
        </div>
      </div>
      <div class="form-row">
  ...
  <div class="panel">
    ...
    <div class="table-scroll">
      <table>
        ...
        <tbody>
        ${rows.length===0 ? `<tr class="empty-row"><td colspan="15">No entries found.</td></tr>` :
          rows.map(e=>`
            <tr>
...
function renderMdTab(){
  ...
  if(mdTab==='items') return simpleListEditor('items', [{key:'name',label:'Item Name'},{key:'itemCode',label:'Item Code'},{key:'description',label:'Description'}]);
...
function attachMasterDataEvents(){
  ...
  const fieldsMap = {
    ...
    items:[{key:'name',label:'Item Name'},{key:'itemCode',label:'Item Code'},{key:'description',label:'Description'}],
...
function attachReportsEvents(){
  ...
  document.getElementById('exportCsv').onclick = ()=>{
    ...
    const header = ['Date','Shift','Site','Location','Machine','ItemCode','Item','ItemDescription','SPM','Qty','Rejected','DowntimeMin','Reason','Operator','Supervisor','OeePct','DowntimePct'];
    ...
    rows.forEach(r=>{
      ...
      lines.push([
        ...
        nameOf(DB.machines,r.machineId), itemRec.itemCode||'', nameOf(DB.items,r.itemId), itemRec.description||'', r.spm, r.qty, r.rejectedQty||0,
