...
function pageAllowedRoutes(){
  ...
  if(PAGE_VIEW === 'entry'){
    const base = canSpm ? ['newentry','entrylog','spm', ...ht] : ['newentry','entrylog', ...ht];
  ...
  if(PAGE_VIEW === 'supervisor') return ['dashboard','newentry','entrylog','spm', ...ht]; // gated at login to supervisor/admin only
  ...
  if(role === 'admin') return ['dashboard','newentry','entrylog','masterdata','reports', ...ht];
  if(role === 'head') return ['dashboard','newentry','entrylog','reports','spm', ...ht];
  ...
  if(role === 'supervisor') return hasDashboard ? ['dashboard','newentry','entrylog','spm', ...ht] : ['newentry','entrylog','spm', ...ht];
  return hasDashboard ? ['dashboard','newentry','entrylog', ...ht] : ['newentry','entrylog', ...ht]; // operator
}
...
const STORE_KEYS = {
  ...
  roleLabels:'prf_role_labels', itemCategories:'prf_item_categories'
};

let DB = { users:[], machines:[], items:[], spm:[], locations:[], sites:[], shifts:[], reasons:[], entries:[], invoices:[], plantmetrics:[], complaints:[], machineholds:[], htfurnaceitems:[], htreasons:[], htentries:[], roleLabels:[], itemCategories:[] };
...
function attachLoginEvents(){
  ...
  if(loginForm) loginForm.onsubmit = async (ev)=>{
    ...
    if(!deptExempt && htPages.includes(PAGE_VIEW) && userDepartment(user)!=='heattreatment'){
      LOGIN_ERR = 'Your account is assigned to Forging, not Heat Treatment. Contact your Admin if this needs to change.';
      render();
      return;
    }
    if(!deptExempt && forgingPages.includes(PAGE_VIEW) && userDepartment(user)!=='forging'){
      LOGIN_ERR = 'Your account is assigned to Heat Treatment, not Forging. Please use your Heat Treatment link instead.';
      render();
      return;
    }
...
/* ================= SHELL ================= */
function navItems(){
  const labels = {dashboard:'Dashboard', newentry:'New Entry', entrylog:'Entry Log', masterdata:'Master Data', reports:'Reports', spm:'SPM', l1:'Executive Dashboard', htnewentry:'HT New Entry', htentrylog:'HT Entry Log'};
  return pageAllowedRoutes().map(key=>({key, label:labels[key]}));
}

function renderShell(){
  const items = navItems();
  const viewLabel = PAGE_VIEW==='entry' ? 'ENTRY TERMINAL' : PAGE_VIEW==='dashboard' ? 'MANAGEMENT VIEW' : PAGE_VIEW==='l1' ? 'EXECUTIVE VIEW' : PAGE_VIEW==='supervisor' ? 'SUPERVISOR VIEW' : PAGE_VIEW==='heattreatment' ? 'HEAT TREATMENT' : PAGE_VIEW==='htentry' ? 'HT ENTRY TERMINAL' : 'ADMIN CONSOLE';
  const labels = {dashboard:'Dashboard', newentry:'New Entry', entrylog:'Entry Log', masterdata:'Master Data', reports:'Reports', spm:'SPM', l1:'Executive Dashboard', htnewentry:'HT New Entry', htentrylog:'HT Entry Log'};
...
function machineStatusGrid(startDate, endDate, dept){
  ...
  const entriesSource = dept==='heattreatment' ? DB.htentries : DB.entries;
  const machineIdField = dept==='heattreatment' ? 'furnaceId' : 'machineId';
...
function renderDeptTabsHtml(){
  if(PAGE_VIEW === 'heattreatment') return ''; // dedicated HT-only URL — no Forging to switch to
  return `
  <div style="display:flex; gap:8px; margin-bottom:18px;">
    <button class="tab-btn ${dashboardDept==='forging'?'active':''}" data-depttab="forging" type="button">Forging</button>
    <button class="tab-btn ${dashboardDept==='heattreatment'?'active':''}" data-depttab="heattreatment" type="button">Heat Treatment</button>
  </div>`;
}
...
function renderMdTab(){
  ...
  if(mdTab==='items') return weightFixBlock() + itemNameMigrationBlock() + itemCategoryMigrationBlock() + simpleListEditor('items', [{key:'itemCode',label:'Item Code'},{key:'category',label:'Category',type:'select',options:DB.itemCategories},{key:'description',label:'Description'},{key:'weightPerPiece',label:'Weight/Pc (g)',type:'number'}]);
...
const DEPARTMENT_OPTIONS = [{id:'forging',name:'Forging'},{id:'heattreatment',name:'Heat Treatment'}];
...
function attachMasterDataEvents(){
  ...
  const fieldsMap = {
    ...
    items:[{key:'itemCode',label:'Item Code'},{key:'category',label:'Category',type:'select',options:DB.itemCategories},{key:'description',label:'Description'},{key:'weightPerPiece',label:'Weight/Pc (g)',type:'number'}],
...
async function boot(){
  ...
  try{
    ...
    if(savedId){
      ...
      const htPages = ['heattreatment','htentry'];
      const forgingPages = ['admin','entry','dashboard','supervisor'];
      const deptOk = user && (deptExempt || (htPages.includes(PAGE_VIEW) ? userDepartment(user)==='heattreatment' : (forgingPages.includes(PAGE_VIEW) ? userDepartment(user)==='forging' : true)));
      const blocked = user && ((PAGE_VIEW === 'dashboard' && !userHasDashboard) || (PAGE_VIEW === 'l1' && !userHasL1) || (PAGE_VIEW === 'supervisor' && !userHasSupervisorPage) || (PAGE_VIEW === 'heattreatment' && !userHasSupervisorPage) || !deptOk);
      if(user && !blocked){
