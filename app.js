...
function lossBreakdown(entries, oeePct){
  ...
  const reasons = Object.keys(lossMap).map(key=>{
  ...
  const reasonPctSum = reasons.reduce((s,r)=>s+r.pct, 0);
  const rawSpeedLossPct = 100 - (oeePct + reasonPctSum);
  // Mathematically this should never be negative (OEE's Availability/Performance/Quality are
  // each capped at 100%, so OEE% + logged-downtime% can't exceed 100%) — but clamp defensively
  // in case of rare floating-point rounding or legacy/inconsistent data, so the breakdown never
  // visually implies more than 100% total.
  const speedLossPct = Math.max(0, rawSpeedLossPct);
  const wasClamped = rawSpeedLossPct < -0.05;
...
function buildMachineSummary(entries){
  ...
  return Object.keys(byMachine).map(mid=>{
    ...
    const lossParts = Object.keys(lossMap)
      ...
      .map(key=>{
        const label = key==='__unspecified' ? 'Unspecified' : nameOf(DB.reasons, key);
        return { label, minutes: lossMap[key] };
      });
