...
function fmtKgTon(kg){ return `${fmt1(kg)} kg <span style="color:var(--ink-dim);">(${fmt3(kg/1000)} t)</span>`; }
...
function attachLoginEvents(){
  ...
  if(loginForm) loginForm.onsubmit = async (ev)=>{
    ...
    LOGIN_ERR=''; SESSION = {id:user.id, name:user.name, role:user.role, locationId:user.locationId||'', canAccessDashboard:user.canAccessDashboard===true}; ROUTE = pageAllowedRoutes()[0];
...
/* ================= SHELL ================= */
function navItems(){
  const labels = {dashboard:'Dashboard', newentry:'New Entry', entrylog:'Entry Log', masterdata:'Master Data', reports:'Reports', spm:'SPM'};
...
function attachShellEvents(){
  document.getElementById('logoutBtn').onclick = ()=>{ if(inactivityTimer) clearTimeout(inactivityTimer); try{ sessionStorage.removeItem('prf_session_userid'); }catch(e){} SESSION=null; LOGIN_MODE='login'; draftPromptDismissed=false; render(); };
...
function renderSummaryMatrix(){
  ...
  const colHeader = (p)=>{
    ...
    return `<th style="min-width:180px;">${p.label}<div style="display:flex; gap:4px; margin-top:6px;">
  ...
  <div class="panel">
    ...
    <div class="table-scroll">
      <table>
        ...
        <tbody>
          <tr><td>Nos.</td>${periods.map((p,i)=>`<td style="font-family:var(--mono);">${cellVal(i,'nos',v=>v.toLocaleString())}</td>`).join('')}</tr>
    ...
    <div class="grid-4">
      ...
      <div class="kpi-card">
        <div style="height:72px;display:flex;flex-direction:column;align-items:center;justify-content:center;">
          ...
          <div style="font-size:13px;font-family:var(--mono);color:var(--ink-dim);margin-top:2px;">${fmt3(agg.totalWeightKg/1000)} t</div>
        </div>
          ...
          ${machineRows.length===0 ? `<tr class="empty-row"><td colspan="7">No entries in this range.</td></tr>` :
            machineRows.map(r=>`
              <tr>
                ...
                <td style="font-family:var(--mono); white-space:nowrap;">${fmt1(r.weightKg)} kg <span style="color:var(--ink-dim); font-size:11.5px;">(${fmt3(r.weightKg/1000)} t)</span></td>
...
function wireUsersCsvImport(){
  ...
  input.onchange = async (ev)=>{
    ...
    for(const row of rows){
      ...
      if(!['operator','supervisor','head','management','admin'].includes(role)) role = '';
      ...
      if(existing){
        ...
        DB.users.push({
          id, name, role, locationId: location ? location.id : '', canAccessDashboard: canDash,
...
function exportUsersCsv(){
  const header = ['UserID','FullName','Role','Location','CanAccessDashboard','SecurityQuestion'];
  const lines = [header.join(',')];
  DB.users.forEach(u=>{
    const autoDash = u.role==='admin' || u.role==='head' || u.role==='management';
    const dashVal = autoDash ? 'auto' : (u.canAccessDashboard ? 'yes' : 'no');
    lines.push([
      u.id, u.name, u.role, u.locationId ? nameOf(DB.locations,u.locationId) : 'All', dashVal, u.securityQ||''
...
function usersEditor(){
  ...
  const csvTemplate = 'userid,fullname,role,location,canaccessdashboard,password,securityquestion,securityanswer\nop01,Ramesh Kumar,operator,' + (DB.locations[0]?DB.locations[0].name:'PRF-I') + ',no,Temp@123,What is your favorite tool?,Hammer';
  const dataUri = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csvTemplate);
  return `
  <div style="display:flex; align-items:center; gap:14px; margin-bottom:16px; flex-wrap:wrap; padding:12px; border:1px dashed var(--line); border-radius:4px;">
    <label class="btn btn-ghost btn-sm" style="cursor:pointer; margin:0;">
    ...
    <a href="${dataUri}" download="accounts_template.csv" class="link-btn">Download template (columns: userid,fullname,role,location,canaccessdashboard,password,securityquestion,securityanswer — role must be operator/supervisor/head/management/admin, location must match an existing Location name (blank = all locations), canaccessdashboard is yes/no (only matters for operator/supervisor))</a>
  </div>
  ...
  <div class="table-scroll">
    <table>
      <thead><tr><th>User ID</th><th>Name</th><th>Role</th><th>Location</th><th>Dashboard</th><th></th></tr></thead>
      <tbody>
      ${list.length===0 ? `<tr class="empty-row"><td colspan="6">${mdSearch.trim() ? 'No matches found.' : 'No accounts yet.'}</td></tr>` :
        list.map(u=>{
          const autoDash = u.role==='admin' || u.role==='head' || u.role==='management';
        ...
        <tr>
          ...
          <td>${u.name}</td>
          <td><span class="tag ${u.role==='admin'?'tag-role-admin':u.role==='operator'?'tag-role-operator':u.role==='head'?'tag-role-head':u.role==='management'?'tag-role-management':''}">${u.role}</span></td>
...
function userModal(){
  ...
  return `
    ...
    <form id="addUserForm">
      ...
      <div class="field"><label>Role</label><select id="nu_role"><option value="operator">Operator</option><option value="supervisor">Supervisor</option><option value="head">Department Head</option><option value="management">Management</option><option value="admin">Admin</option></select>
        <div class="helptext">Operator: submit entries only. Supervisor: full edit/delete on any entry. Department Head: Supervisor rights + Reports + all locations, but no Master Data. Management: view-only Dashboard, Entry Log &amp; Reports across all locations — can't submit entries or edit Master Data. Admin: full access.</div>
      </div>
...
function attachEditAccountModalEvents(userId){
  ...
  document.getElementById('editAccountForm').onsubmit = async (ev)=>{
    ...
    await save('users');
    closeModal();
    if(userId === SESSION.id){ SESSION.name = newName; SESSION.canAccessDashboard = user.canAccessDashboard; }
    render();
...
function pageReports(){
  ...
  <div class="panel">
    ...
    <div class="grid-4" style="margin-bottom:20px;">
      ...
      <div class="kpi-card"><div class="kpi-label">Total Weight</div><div class="kpi-value" style="font-size:18px;">${fmt1(agg.totalWeightKg)} kg</div><div style="font-size:12px;font-family:var(--mono);color:var(--ink-dim);margin-top:2px;">${fmt3(agg.totalWeightKg/1000)} t</div></div>
    </div>
...
async function boot(){
  ...
  try{
    ...
    if(savedId){
      ...
      if(user && !(PAGE_VIEW === 'dashboard' && !userHasDashboard)){
        SESSION = {id:user.id, name:user.name, role:user.role, locationId:user.locationId||'', canAccessDashboard:user.canAccessDashboard===true};
