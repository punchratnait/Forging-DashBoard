  ...
  const perLocation = locsToShow.map(loc=>{
    ...
    const machines = machinesInLoc.map(m=>{
      const statusByDate = dates.map(dt=>{
        const entered = DB.entries.some(e=>e.date===dt && e.locationId===loc.id && e.machineId===m.id);
...
function attachReportsEvents(){
  ...
  document.getElementById('exportCsv').onclick = ()=>{
    ...
    rows.sort((a,b)=> nameOf(DB.machines,a.machineId).localeCompare(nameOf(DB.machines,b.machineId)) || a.date.localeCompare(b.date));
