  ...
  const perLocation = locsToShow.map(loc=>{
    ...
    const machines = machinesInLoc.map(m=>{
      const statusByDate = dates.map(dt=>{
        ...
        return { date: dt, entered };
      });
      const pendingDays = statusByDate.filter(s=>!s.entered).length;
      return { id:m.id, code:m.machineCode||m.name, fullLabel:machineLabel(m.id), statusByDate, pendingDays };
    });
    const totalCells = machines.length * dates.length;
    const pendingCells = machines.reduce((s,m)=>s+m.pendingDays, 0);
    return { location: loc, machines, pendingCells, totalCells };
...
function renderMachineStatusSection(){
  ...
  if(dates.length <= 1){
    ...
    const tile = (m)=>{
      const entered = m.statusByDate[0] ? m.statusByDate[0].entered : false;
      return `<div class="machine-status-tile ${entered?'entered':'pending'}" title="${m.fullLabel} — ${entered?'Entered':'Pending'} (${d})">${m.code}</div>`;
    };
          ...
          <tbody>
            ${r.machines.map(m=>`
              <tr>
                ...
                ${m.statusByDate.map(s=>`<td style="text-align:center;"><span class="grid-dot ${s.entered?'entered':'pending'}" title="${m.fullLabel} — ${s.entered?'Entered':'Pending'} (${s.date})"></span></td>`).join('')}
              </tr>`).join('')}
          </tbody>
        </table>
      </div>
  ...
  return `
    ...
    <div style="font-size:12.5px; color:var(--ink-dim); margin-bottom:14px;">
      <b style="color:${activePending>0?'var(--red)':'var(--green)'};">${activePending}</b> of ${activeTotal} machine-day${dates.length>1?'s':''} pending${missingEntriesPlant?` at ${nameOf(DB.locations,missingEntriesPlant)}`:''}
    </div>
    ${body}
    <div class="helptext" style="margin-top:14px;">Green = at least one entry logged. Red = zero entries — either genuinely idle, or never logged. ${dates.length>1 ? 'Each column is one date in the selected range.' : 'Refreshes automatically every 60 seconds while on this page.'}</div>
  `;
}
