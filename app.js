...
function machinesMissingYesterday(){
  ...
  return locsToShow.map(loc=>{
    ...
    const machinesWithEntry = new Set(
      DB.entries.filter(e=>e.date===yesterday && e.locationId===loc.id).map(e=>e.machineId)
    );
    const missing = machinesInLoc.filter(m=>!machinesWithEntry.has(m.id));
    return { location: loc, total: machinesInLoc.length, missing };
...
function renderMissingEntriesPanel(){
  const rows = machinesMissingYesterday();
  const yesterday = yesterdayStr();
  ...
  const chipsForRow = (r)=> r.missing.map(m=>
    `<span class="tag" style="border-color:var(--red); color:var(--red);" title="${machineLabel(m.id)}">${m.machineCode || m.name}</span>`
  ).join(' ');

  const plantSelector = showSelector ? `
    <div style="display:flex; gap:8px; margin-bottom:16px; flex-wrap:wrap;">
      <button class="tab-btn ${missingEntriesPlant===''?'active':''}" data-missingplant="" type="button">Consolidated</button>
      ${rows.map(r=>`<button class="tab-btn ${missingEntriesPlant===r.location.id?'active':''}" data-missingplant="${r.location.id}" type="button">${r.location.name}</button>`).join('')}
    </div>` : '';

  const activeMissing = activeRows.reduce((s,r)=>s+r.missing.length, 0);
  const body = activeMissing === 0
    ? `<div class="helptext">No machines missing an entry for ${yesterday}${missingEntriesPlant ? ` at ${nameOf(DB.locations,missingEntriesPlant)}` : ''}. Nothing to flag.</div>`
    : `<div style="display:flex; flex-wrap:wrap; gap:8px;">
        ${activeRows.filter(r=>r.missing.length>0).map(r=>
          !missingEntriesPlant && rows.length>1
            ? `<div style="width:100%; font-size:11px; color:var(--ink-dim); margin-top:8px;">${r.location.name}</div>${chipsForRow(r)}`
            : chipsForRow(r)
        ).join('')}
      </div>`;
  ...
  <div class="panel" style="margin-top:18px;">
    <div class="panel-title"><span class="bar"></span>Machines Without Yesterday's Entry (${yesterday})</div>
    ${plantSelector}
    ${body}
    ${totalMissing > 0 ? `<div class="helptext" style="margin-top:14px;">A machine here had zero production entries dated ${yesterday} — either genuinely idle all day, or the entry was never logged. Worth a quick check with the relevant Supervisor.</div>` : ''}
  </div>
  `;
}
