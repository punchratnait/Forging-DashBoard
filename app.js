    ...
    <form id="htEntryForm">
      ...
      <div class="form-row">
        <div class="field item-suggest-wrap"><label>Product Code</label>
          ...
          <select id="ht_item" style="display:none;">${renderItemOptions(DB.items, d.itemId, 'Select item')}</select>
...
function attachHtNewEntryEvents(){
  ...
  itemSearchInput.onfocus = ()=>{ if(itemSearchInput.value.trim()){ itemSuggestBox.innerHTML = itemSuggestionsHtml(itemSearchInput.value); itemSuggestBox.style.display=''; } };
  itemSearchInput.onblur = ()=>{ setTimeout(()=>{ itemSuggestBox.style.display='none'; }, 150); };
  itemSuggestBox.onmousedown = (ev)=>{
    const row = ev.target.closest('.item-suggest-row');
...
function renderMdTab(){
  ...
  if(mdTab==='htfurnaceitems') return simpleListEditor('htfurnaceitems', htFurnaceItemFields());
...
function htFurnaceItemFields(){
  const furnaces = DB.machines.filter(m=>machineDepartment(m)==='heattreatment').map(m=>({id:m.id, name:machineLabel(m.id)}));
  return [
    {key:'furnaceId', label:'Furnace', type:'select', options: furnaces},
    {key:'itemId', label:'Product Code', type:'select', options: DB.items.map(i=>({id:i.id, name:itemLabel(i)}))}
  ];
}
...
function attachMasterDataEvents(){
  ...
  const fieldsMap = {
    ...
    htfurnaceitems: htFurnaceItemFields(),
