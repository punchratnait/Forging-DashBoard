...
function htEntryRowToObj(row){
  return {
    ...
    qtyKg: row.qty_kg, jobChangeCount: row.job_change_count, jobChangeLossMinutes: row.job_change_loss_minutes,
...
function htEntryObjToRow(e){
  return {
    ...
    qty_kg: e.qtyKg, job_change_count: e.jobChangeCount, job_change_loss_minutes: e.jobChangeLossMinutes,
...
function htSummaryMatrixData(){
  ...
  const cells = periods.map(p=>{
    ...
    if(activePlant) entries = entries.filter(e=>e.locationId===activePlant);
    const totalKg = entries.reduce((s,e)=>s+(Number(e.qtyKg)||0), 0);
    const erpDoneKg = entries.reduce((s,e)=>s+(Number(e.erpDoneKg)||0), 0);
    return { totalKg, erpDoneKg, pendingKg: Math.max(totalKg-erpDoneKg, 0) };
  });
...
function pageHtDashboardContent(){
  ...
  const totalKgMtd = mtdHt.reduce((s,e)=>s+(Number(e.qtyKg)||0), 0);
...
function htTonnageByFurnace(startDate, endDate){
  ...
  const byFurnace = {};
  entries.forEach(e=>{
    byFurnace[e.furnaceId] = (byFurnace[e.furnaceId]||0) + (Number(e.qtyKg)||0);
  });
  const rows = Object.keys(byFurnace).map(fid=>({
    machineId: fid, code: machineLabel(fid), fullLabel: nameOf(DB.machines, fid), tonnageKg: byFurnace[fid]
  })).sort((a,b)=>b.tonnageKg-a.tonnageKg);
  const consolidatedKg = rows.reduce((s,r)=>s+r.tonnageKg, 0);
  return { rows, consolidatedKg };
}
...
function pageHtSupervisorDashboardContent(){
  ...
  <div class="panel" style="margin-top:18px;">
    ...
    <div class="kpi-card" style="margin-bottom:16px; max-width:280px;">
      <div class="kpi-label">Consolidated (All Furnaces)</div>
    ...
    <div class="table-scroll">
      <table>
        <thead><tr><th>Furnace</th><th>Weight</th></tr></thead>
        <tbody>
...
function blankHtDraft(){
  ...
  return {
    ...
    furnaceId:'', itemId:'', jobOrderNumber:'', qtyKg:'', erpDoneKg:'',
...
function renderHtExtraItemsHtml(items){
  ...
  return items.map(it=>{
    ...
    <div class="ht-extra-item-row" data-itemrowid="${it.id}" style="display:flex; gap:10px; align-items:flex-end; margin-bottom:14px; flex-wrap:wrap; padding-bottom:10px; border-bottom:1px dashed var(--line);">
      ...
      <div class="field" style="min-width:120px; margin-bottom:0;"><label>Qty (Kg)</label><input type="number" min="0" step="0.01" class="htei-qty" data-itemrowid="${it.id}" value="${it.qtyKg||''}"></div>
      <div class="field" style="min-width:120px; margin-bottom:0;"><label>ERP Done (Kg)</label><input type="number" min="0" step="0.01" class="htei-erp" data-itemrowid="${it.id}" value="${it.erpDoneKg||''}"></div>
...
function pageHtNewEntry(){
  ...
  <div class="panel">
    <form id="htEntryForm">
      ...
      <div class="form-row">
        <div class="field"><label>Production Qty (Kg)</label><input type="number" min="0" step="0.01" id="ht_qty" value="${d.qtyKg}" required></div>
        <div class="field"><label>ERP Done (Kg)</label><input type="number" min="0" step="0.01" id="ht_erp_done" value="${d.erpDoneKg}"></div>
        <div class="field"><label>Pending Qty (Kg)</label><input type="text" id="ht_pending_display" value="${fmt1(pendingKg)}" readonly style="opacity:0.7; ${pendingKg>0?'color:var(--amber);':'color:var(--green);'}"></div>
      </div>
...
function attachHtNewEntryEvents(){
  ...
  if(addHtExtraItemBtn){
    addHtExtraItemBtn.onclick = ()=>{
      htEntryDraft.items.push({id: uid('htei'), itemId:'', jobOrderNumber:'', qtyKg:0, erpDoneKg:0});
      extraItemsContainer.innerHTML = renderHtExtraItemsHtml(htEntryDraft.items);
      wireHtExtraItemTypeaheads();
    };
  }
...
function pageHtEntryLog(){
  ...
  <div class="panel">
    ...
    <div style="font-size:12.5px; color:var(--ink-dim); margin-bottom:14px;">
      <b style="color:var(--amber);">${rows.length}</b> entr${rows.length===1?'y':'ies'} found${rows.length>0 ? ` · <b style="color:var(--ink);">${fmt1(rows.reduce((s,e)=>s+(Number(e.qtyKg)||0),0))} kg</b> total` : ''}
    </div>
    <div class="table-scroll">
      <table>
        <thead><tr>
          <th>Date</th><th>Shift</th><th>Site</th><th>Location</th><th>Furnace</th><th>Job Order No.</th><th>Product</th>
          <th>Qty (Kg)</th><th>ERP Done</th><th>Pending</th><th>Job Chg</th><th>Job Loss (min)</th><th>D/T Min</th><th>Reason</th><th>${roleLabel('heattreatment','operator')}</th><th>${roleLabel('heattreatment','supervisor')}</th><th></th>
        </tr></thead>
        <tbody>
          ${rows.length===0 ? `<tr class="empty-row"><td colspan="17">No HT entries found.</td></tr>` :
            rows.map(e=>{
              ...
              <tr>
                ...
                <td style="font-family:var(--mono);">${fmt1(e.qtyKg)}</td>
                <td style="font-family:var(--mono);">${fmt1(e.erpDoneKg)}</td>
                <td style="font-family:var(--mono); color:${pendingKg>0?'var(--amber)':'var(--green)'};">${fmt1(pendingKg)}</td>
...
function attachHtEntryLogEvents(){
  ...
  if(exportBtn) exportBtn.onclick = ()=>{
    ...
    const header = ['Date','Shift','Site','Location','Furnace','Job Order No.','Product','Qty (Kg)','ERP Done','Pending','Job Changes','Job Loss (min)','Down Time (min)','Reason',roleLabel('heattreatment','operator'),roleLabel('heattreatment','supervisor')];
    ...
    rows.forEach(x=>{
      ...
      const row = [
        ...
        fmt1(x.qtyKg), fmt1(x.erpDoneKg), fmt1(pendingKg), jcCount, jcLoss, dtMin,
