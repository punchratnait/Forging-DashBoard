...
function bulkEditModal(dbKey, fields){
  ...
  return `
    ...
    <p class="helptext" style="margin-bottom:14px;">Choose one field and a new value — it will be applied to all ${count} selected record${count===1?'':'s'} at once. This cannot be undone in bulk (you'd need to fix records individually).</p>
    <form id="bulkEditForm">
      ...
      <div id="be_value_wrap">${bulkEditValueInput(fields[0])}</div>
      <div class="form-actions">
...
function bulkEditValueInput(field){
  if(!field) return '';
...
function attachBulkEditModalEvents(dbKey, fields){
  ...
  fieldSelect.onchange = ()=>{
    ...
    valueWrap.innerHTML = bulkEditValueInput(field);
  };
  document.getElementById('bulkEditForm').onsubmit = async (ev)=>{
    ...
    render();
    alert(`Updated ${field.label} on ${changed} record${changed===1?'':'s'}.`);
  };
}
