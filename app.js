  ...
  const perLocation = locsToShow.map(loc=>{
    ...
    const machines = machinesInLoc.map(m=>{
      const statusByDate = dates.map(dt=>{
        const entered = DB.entries.some(e=>e.date===dt && e.machineId===m.id);
...
function renderLocationMismatchPanel(){
  ...
  if(mismatches.length === 0 && dupeGroups.length === 0) return '';
  return `
  <div class="panel" style="margin-top:18px; border-color:var(--amber);">
    <div class="panel-title" style="color:var(--amber);"><span class="bar" style="background:var(--amber);"></span>Data Integrity: Entry Location Mismatches Found</div>
    ${dupeGroups.length > 0 ? `
