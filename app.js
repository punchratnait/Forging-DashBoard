...
function machineStatusWall(){
  const yesterday = yesterdayStr();
  const restrictedLoc = effectiveLocationId();
  const locsToShow = restrictedLoc ? DB.locations.filter(l=>l.id===restrictedLoc) : DB.locations;
  return locsToShow.map(loc=>{
    const machinesInLoc = DB.machines.filter(m=>m.location===loc.id);
    const enteredIds = new Set(
      DB.entries.filter(e=>e.date===yesterday && e.locationId===loc.id).map(e=>e.machineId)
    );
    const machines = machinesInLoc.map(m=>({ id:m.id, code:m.machineCode||m.name, fullLabel:machineLabel(m.id), entered:enteredIds.has(m.id) }));
    const pending = machines.filter(m=>!m.entered).length;
    return { location: loc, machines, pending, total: machines.length };
  }).filter(r=>r.total > 0); // skip locations with no machines assigned at all
}

function renderMissingEntriesPanel(){
  const rows = machineStatusWall();
  const yesterday = yesterdayStr();
  if(rows.length === 0) return '';
  const totalPending = rows.reduce((s,r)=>s+r.pending, 0);
  const restrictedLoc = effectiveLocationId();
  const showSelector = !restrictedLoc && rows.length > 1;

  const activeRows = missingEntriesPlant
    ? rows.filter(r=>r.location.id===missingEntriesPlant)
    : rows; // Consolidated = every plant together
  const activePending = activeRows.reduce((s,r)=>s+r.pending, 0);
  const activeTotal = activeRows.reduce((s,r)=>s+r.total, 0);

  const tile = (m)=> `<div class="machine-status-tile ${m.entered?'entered':'pending'}" title="${m.fullLabel} — ${m.entered?'Entered':'Pending'} (${yesterday})">${m.code}</div>`;
  ...
  const plantSelector = showSelector ? `
    <div style="display:flex; gap:8px; margin-bottom:16px; flex-wrap:wrap;">
      ...
      ${rows.map(r=>`<button class="tab-btn ${missingEntriesPlant===r.location.id?'active':''}" data-missingplant="${r.location.id}" type="button">${r.location.name}${r.pending>0?`<span class="pending-badge">${r.pending}</span>`:''}</button>`).join('')}
    </div>` : '';

  const body = activeRows.map(r=>`
    ${(!missingEntriesPlant && rows.length>1) ? `<div style="width:100%; font-size:11px; color:var(--ink-dim); margin:${r===activeRows[0]?'0':'14px'} 0 8px;">${r.location.name} — ${r.pending}/${r.total} pending</div>` : ''}
    <div style="display:flex; flex-wrap:wrap; gap:8px;">
      ${r.machines.map(tile).join('')}
    </div>
  `).join('');

  return `
  <div class="panel" style="margin-top:18px;">
    <div class="panel-title"><span class="bar"></span>Machine Status — Yesterday's Entry (${yesterday})</div>
    ${plantSelector}
    <div style="font-size:12.5px; color:var(--ink-dim); margin-bottom:14px;">
      <b style="color:${activePending>0?'var(--red)':'var(--green)'};">${activePending}</b> of ${activeTotal} machines pending${missingEntriesPlant?` at ${nameOf(DB.locations,missingEntriesPlant)}`:''}
    </div>
    ${body}
    <div class="helptext" style="margin-top:14px;">Green = at least one entry logged for ${yesterday}. Red = zero entries — either genuinely idle all day, or the entry was never logged. Refreshes automatically every 60 seconds.</div>
  </div>
  `;
}
