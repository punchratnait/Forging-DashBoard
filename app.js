...
function pageAllowedRoutes(){
  ...
  if(PAGE_VIEW === 'supervisor') return ['dashboard','newentry','entrylog','spm']; // gated at login to supervisor/admin only
...
const STORE_KEYS = {
  ...
  machineholds:'prf_machine_holds'
};

let DB = { users:[], machines:[], items:[], spm:[], locations:[], sites:[], shifts:[], reasons:[], entries:[], invoices:[], plantmetrics:[], complaints:[], machineholds:[] };
...
function machineStatusGrid(startDate, endDate){
  ...
  const perLocation = locsToShow.map(loc=>{
    const machinesInLoc = DB.machines.filter(m=>m.location===loc.id);
