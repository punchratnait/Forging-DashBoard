...
function pageAllowedRoutes(){
  ...
  if(PAGE_VIEW === 'htentry') return ['htnewentry','htentrylog']; // dedicated HT Entry URL — gated at login to supervisor/admin only
...
function attachLoginEvents(){
  ...
  if(loginForm) loginForm.onsubmit = async (ev)=>{
    ...
    if(PAGE_VIEW === 'htentry' && !(user.role==='supervisor' || user.role==='admin')){
      LOGIN_ERR = 'This Heat Treatment Entry link is for Supervisor and Admin accounts only. Please use your regular Entry link instead.';
      render();
      return;
    }
...
function attachHtNewEntryEvents(){
  ...
  document.getElementById('htEntryForm').onsubmit = async (ev)=>{
    ...
    const mainMapping = lookupHtMapping(furnaceSel.value, itemSel.value);
    if(!mainMapping){
      alert('This Product Code is not mapped to this Furnace. Please add it under Master Data → HT Item-Furnace Map first, or ask your Supervisor.');
      return;
    }
    ...
    for(const it of validExtraItems){
      const item = byId(DB.items, it.itemId);
      if(!lookupHtMapping(furnaceSel.value, it.itemId)){
        alert(`${item?itemLabel(item):'One of the additional items'} is not mapped to this Furnace. Please add it under Master Data → HT Item-Furnace Map first, or remove it.`);
        return;
      }
          ...
          ${rows.length===0 ? `<tr class="empty-row"><td colspan="17">No HT entries found.</td></tr>` :
            rows.map(e=>{
              ...
              <tr>
                ...
                <td style="font-family:var(--mono);">${e.jobChangeCount||0}</td>
                <td style="font-family:var(--mono);">${e.jobChangeLossMinutes||0}</td>
  ...
  if(exportBtn) exportBtn.onclick = ()=>{
    ...
    rows.forEach(x=>{
      ...
      const row = [
        ...
        fmt1(x.qtyKg), fmt1(x.erpDoneKg), fmt1(pendingKg), x.jobChangeCount||0, x.jobChangeLossMinutes||0, dtMin,
...
async function boot(){
  ...
  try{
    ...
    if(savedId){
      ...
      const blocked = user && ((PAGE_VIEW === 'dashboard' && !userHasDashboard) || (PAGE_VIEW === 'l1' && !userHasL1) || (PAGE_VIEW === 'supervisor' && !userHasSupervisorPage) || (PAGE_VIEW === 'heattreatment' && !userHasSupervisorPage) || (PAGE_VIEW === 'htentry' && !userHasSupervisorPage) || !deptOk);
      if(user && !blocked){
