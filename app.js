...
function blankHtDraft(){
  ...
  return {
    ...
    furnaceId:'', itemId:'', qtyKg:'', erpDoneKg:'',
  ...
  <div class="panel">
    <form id="htEntryForm">
      <div class="form-row">
        ...
        <div class="field"><label>Shift</label><select id="ht_shift" required>${opts(DB.shifts, d.shiftId, 'Select shift')}</select></div>
...
function attachHtNewEntryEvents(){
  ...
  document.getElementById('htEntryForm').onsubmit = async (ev)=>{
    ...
    const mapping = lookupHtMapping(furnaceSel.value, itemSel.value);
    if(!mapping){
    ...
    if(htEditingEntryId){
      ...
      if(idx>=0) DB.htentries[idx] = rec;
      ok = await updateHtEntryRow(rec);
    } else {
      DB.htentries.push(rec);
      ok = await insertHtEntries([rec]);
    }
...
function pageHtEntryLog(){
  ...
  if(htLogFilters.search && htLogFilters.search.trim()){
    ...
    rows = rows.filter(e=>{
      ...
      const item = byId(DB.items, e.itemId) || {};
      const haystack = [furnace.name, furnace.machineCode, item.description, item.itemCode, e.operatorName, e.supervisorName].join(' ').toLowerCase();
      return haystack.includes(q);
    });
  }
  ...
  <div class="panel">
    <div class="filter-bar">
      <div class="field" style="min-width:200px;"><label>Search</label><input type="text" id="htlf_search" placeholder="Furnace, product, operator..." value="${htLogFilters.search}"></div>
      ...
      <div class="field"><label>Shift</label><select id="htlf_shift"><option value="">All</option>${DB.shifts.map(m=>`<option value="${m.id}" ${htLogFilters.shiftId===m.id?'selected':''}>${m.name}</option>`).join('')}</select></div>
    ...
    <div class="table-scroll">
      <table>
        <thead><tr>
          <th>Date</th><th>Shift</th><th>Site</th><th>Location</th><th>Furnace</th><th>Product</th>
        ...
        <tbody>
          ${rows.length===0 ? `<tr class="empty-row"><td colspan="16">No HT entries found.</td></tr>` :
            rows.map(e=>{
              ...
              <tr>
                ...
                <td>${nameOf(DB.sites, e.siteId)}</td>
                <td>${nameOf(DB.locations, e.locationId)}</td>
                <td title="${nameOf(DB.machines, e.furnaceId)}">${machineLabel(e.furnaceId)}</td>
...
function attachHtEntryLogEvents(){
  ...
  document.querySelectorAll('[data-htedit]').forEach(btn=>{
    btn.onclick = ()=>{
      ...
      if(!rec) return;
      htEntryDraft = {...rec, downtimeBlocks: (rec.downtimeBlocks||[]).map(b=>({...b, id: b.id||uid('htdtb')}))};
