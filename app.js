...
async function seedIfEmpty(){
  ...
  if(DB.shifts.length===0){
    DB.shifts.push({id:uid('sh'),name:'Shift A (6-2)',startTime:'06:00',durationMinutes:480});
    DB.shifts.push({id:uid('sh'),name:'Shift B (2-10)',startTime:'14:00',durationMinutes:480});
    DB.shifts.push({id:uid('sh'),name:'Shift C (10-6)',startTime:'22:00',durationMinutes:480});
    changed = true;
  }
  ...
  <div class="shell">
    ...
    <div class="sidebar" id="sidebarPanel">
      ...
      <div class="side-foot">
        ...
        ${SESSION.locationId ? `<div style="font-size:10.5px;color:var(--ink-dim);margin-top:2px;">📍 ${nameOf(DB.locations,SESSION.locationId)}</div>` : ''}
...
function pageDashboard(){
  ...
  const scoped = SESSION.locationId ? DB.entries.filter(e=>e.locationId===SESSION.locationId) : DB.entries;
  ...
  <div class="panel" style="margin-top:18px;">
    ...
    <div class="table-scroll">
      <table>
        ...
        <tbody>
          ${machineRows.length===0 ? `<tr class="empty-row"><td colspan="6">No entries in this range.</td></tr>` :
            machineRows.map(r=>`
              <tr>
  ...
  <div class="footer-note">Formulas: OEE % = Availability × Performance × Quality × 100 (Run Time/Available Time, Actual/Standard Qty, Good Qty/Actual Qty). Down Time % = Down Time ÷ Available Time × 100. Available Time comes from the Shift's configured duration in Master Data. Production Weight = entered Weight where available, otherwise Qty × Item's Weight/Pc, shown in kg. Share your exact formulas any time and these can be adjusted.</div>
  `;
}
...
function blankDraft(){
  return {
    date: todayStr(), shiftId:'', siteId:'', locationId: (SESSION && SESSION.locationId) || '', machineId:'', itemId:'',
    spm:'', qty:'', weight:'', rejectedQty:'', downtimeBlocks:[], operatorName:''
  };
}
...
function pageNewEntry(){
  ...
  <div class="panel">
    <form id="entryForm">
      ...
      <div class="form-row">
        <div class="field"><label>Location</label><select id="f_location" required ${SESSION.locationId?'disabled':''}>${opts(SESSION.locationId ? DB.locations.filter(l=>l.id===SESSION.locationId) : DB.locations, d.locationId, 'Select location')}</select>
          ${SESSION.locationId ? `<div class="helptext">Locked to your assigned location: ${nameOf(DB.locations,SESSION.locationId)}.</div>` : ''}
        </div>
      ...
      <div class="grid-3" style="margin-bottom:18px;">
        ...
        <div class="kpi-card"><div class="kpi-label">Working Time (Shift − Down Time)</div><div class="kpi-value" id="workingTimeDisplay" style="font-size:18px;">—</div></div>
...
function attachNewEntryEvents(){
  ...
  const recomputeSummary = ()=>{
    ...
    const shiftMinutes = shift ? shift.durationMinutes : 0;
  ...
  document.getElementById('entryForm').onsubmit = async (ev)=>{
    ...
    const rec = {
      ...
      // Preserve who originally created this entry, even if someone else edits it now
      supervisorId: existing ? existing.supervisorId : SESSION.id,
      supervisorName: existing ? existing.supervisorName : SESSION.name,
...
function pageEntryLog(){
  ...
  if(SESSION.locationId) rows = rows.filter(e=>e.locationId===SESSION.locationId);
  ...
  const canManage = ()=> SESSION.role==='admin' || SESSION.role==='supervisor'; // operators can submit entries but cannot edit/delete any
...
function renderMdTab(){
  ...
  if(mdTab==='items') return simpleListEditor('items', [{key:'name',label:'Item Name'},{key:'itemCode',label:'Item Code'},{key:'description',label:'Description'},{key:'weightPerPiece',label:'Weight/Pc (g)',type:'number'}]);
  ...
  if(mdTab==='shifts') return simpleListEditor('shifts', [{key:'name',label:'Shift Name'},{key:'startTime',label:'Start Time',type:'time'},{key:'durationMinutes',label:'Duration (min)',type:'number'}]);
...
function simpleListEditor(dbKey, fields){
  ...
  <div class="field" style="max-width:320px; margin-bottom:16px;"><label>Search</label><input type="text" id="mdSearchInput" placeholder="Search ${MD_TABS.find(t=>t.key===mdTab).label.toLowerCase()}..." value="${mdSearch}"></div>
...
function spmEditor(){
  ...
  <div class="field" style="max-width:320px; margin-bottom:16px;"><label>Search</label><input type="text" id="mdSearchInput" placeholder="Search by machine or item code..." value="${mdSearch}"></div>
    ...
    <table>
      ...
      ${list.length===0 ? `<tr class="empty-row"><td colspan="5">${mdSearch.trim() ? 'No matches found.' : 'No accounts yet.'}</td></tr>` :
        ...
        <tr>
          ...
          <td>${u.name}</td>
          <td><span class="tag ${u.role==='admin'?'tag-role-admin':u.role==='operator'?'tag-role-operator':''}">${u.role}</span></td>
...
function attachMasterDataEvents(){
  document.querySelectorAll('[data-tab]').forEach(btn=>{
    btn.onclick = ()=>{ mdTab = btn.dataset.tab; mdSearch = ''; render(); };
  });
  ...
  const fieldsMap = {
    ...
    shifts:[{key:'name',label:'Shift Name'},{key:'startTime',label:'Start Time',type:'time'},{key:'durationMinutes',label:'Duration (min)',type:'number'}],
  ...
  if(addForm) addForm.onsubmit = async (ev)=>{
    ...
    fields.forEach(f=>{
      ...
      rec[f.key] = f.type==='number' ? Number(el.value)||0 : el.value;
...
function userModal(){
  ...
  return `
    ...
    <form id="addUserForm">
      ...
      <div class="field"><label>Role</label><select id="nu_role"><option value="operator">Operator</option><option value="supervisor">Supervisor</option><option value="admin">Admin</option></select>
        <div class="helptext">Operator: can submit New Entries, cannot edit/delete. Supervisor &amp; Admin: full edit/delete on any entry.</div>
      </div>
