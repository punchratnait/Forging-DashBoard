...
function buildMachineSummary(entries){
  ...
  return Object.keys(byMachine).map(mid=>{
    ...
    return { mid, plannedQty:Math.round(plannedQty), actualQty, oeePct:a.oeePct, downtimePct:a.downtimePct, lossText: lossParts.length ? lossParts.join(', ') : '—' };
...
function pageDashboard(){
  ...
  <div class="panel" style="margin-top:18px;">
    ...
    <div class="table-scroll">
      <table>
        <thead><tr><th>Machine</th><th>Planned Qty</th><th>Actual Qty</th><th>OEE %</th><th>Losses (Reason: Minutes)</th></tr></thead>
        <tbody>
          ${machineRows.length===0 ? `<tr class="empty-row"><td colspan="5">No entries in this range.</td></tr>` :
            machineRows.map(r=>`
              <tr>
  ...
  <div class="panel">
    <form id="entryForm">
      ...
      <div class="form-row">
        ...
        <div class="field"><label>Machine</label><select id="f_machine" required>${opts(machinesForLocation(d.locationId), d.machineId, 'Select machine')}</select>
      ...
      <div class="form-row">
        ...
        <div class="field"><label>Weight Produced (kg, optional)</label><input type="number" min="0" step="any" id="f_weight" value="${d.weight}">
          <div class="helptext">If the item has a Weight/Pc set in Master Data, Qty auto-calculates from this.</div>
        </div>
...
function attachNewEntryEvents(){
  ...
  const updateQtyFromWeight = ()=>{
    const item = byId(DB.items, itemSel.value);
    const wpp = item ? Number(item.weightPerPiece)||0 : 0;
    const weight = Number(weightInput.value)||0;
    if(wpp > 0 && weight > 0){
      qtyInput.value = Math.round(weight / wpp);
    }
  };
  locationSel.onchange = ()=>{
    machineSel.innerHTML = renderOptions(machinesForLocation(locationSel.value), '', 'Select machine');
  ...
  itemSel.onchange = ()=>{ updateSpm(); updateQtyFromWeight(); updateItemInfo(); };
  weightInput.oninput = updateQtyFromWeight;
...
function pageEntryLog(){
  ...
  <div class="panel">
    ...
    <div class="table-scroll">
      <table>
        <thead><tr>
          <th>Date</th><th>Shift</th><th>Site</th><th>Location</th><th>Machine</th><th>Item Code</th><th>Item</th>
          <th>SPM</th><th>Weight (kg)</th><th>Qty</th><th>Rej.</th><th>D/T Min</th><th>Reason</th><th>Operator</th><th>Supervisor</th><th></th>
        </tr></thead>
        <tbody>
        ${rows.length===0 ? `<tr class="empty-row"><td colspan="16">No entries found.</td></tr>` :
          rows.map(e=>`
            <tr>
...
function renderMdTab(){
  if(mdTab==='machines') return simpleListEditor('machines', [{key:'name',label:'Machine Name'},{key:'site',label:'Site',type:'select',options:DB.sites},{key:'location',label:'Location',type:'select',options:DB.locations}]);
  if(mdTab==='items') return simpleListEditor('items', [{key:'name',label:'Item Name'},{key:'itemCode',label:'Item Code'},{key:'description',label:'Description'},{key:'weightPerPiece',label:'Weight/Pc (kg)',type:'number'}]);
...
function attachMasterDataEvents(){
  ...
  const fieldsMap = {
    machines:[{key:'name',label:'Machine Name'},{key:'site',label:'Site',type:'select',options:DB.sites},{key:'location',label:'Location',type:'select',options:DB.locations}],
    items:[{key:'name',label:'Item Name'},{key:'itemCode',label:'Item Code'},{key:'description',label:'Description'},{key:'weightPerPiece',label:'Weight/Pc (kg)',type:'number'}],
  ...
  <div class="panel">
    ...
    <div class="table-scroll">
      <table>
        <thead><tr><th>Machine</th><th>Planned Qty</th><th>Actual Qty</th><th>OEE %</th><th>Losses (Reason: Minutes)</th></tr></thead>
        <tbody>
          ${machineRows.length===0 ? `<tr class="empty-row"><td colspan="5">No data in this range.</td></tr>` :
            machineRows.map(r=>`
              <tr>
                <td>${nameOf(DB.machines,r.mid)}</td>
                <td style="font-family:var(--mono);">${r.plannedQty}</td>
    ...
    <div class="helptext" style="margin-top:10px;">Planned Qty = SPM × full shift duration (before any losses). Actual Qty = what was actually produced. Losses lists down time minutes grouped by reason, largest first.</div>
  </div>
  `;
}
...
function attachReportsEvents(){
  ...
  document.getElementById('exportCsv').onclick = ()=>{
    ...
    const header = ['Date','Shift','Site','Location','Machine','ItemCode','Item','ItemDescription','SPM','Weight','Qty','Rejected','DowntimeMin','Reason','Operator','Supervisor','OeePct','DowntimePct'];
    ...
    rows.forEach(r=>{
      ...
      lines.push([
        ...
        nameOf(DB.machines,r.machineId), itemRec.itemCode||'', nameOf(DB.items,r.itemId), itemRec.description||'', r.spm, r.weight||0, r.qty, r.rejectedQty||0,
