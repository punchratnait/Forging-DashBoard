...
function blankDraft(){
  return {
    ...
    spm:'', qty:'', weight:'', rejectedQty:'', downtimeMinutes:'0', downtimeReasonId:'', operatorName:''
  };
}
...
function attachNewEntryEvents(){
  ...
  itemSel.onchange = ()=>{ updateSpm(); updateItemInfo(); updateWeightFromQty(); };
  qtyInput.oninput = updateWeightFromQty;
  ...
  if(logFilters.search && logFilters.search.trim()){
    ...
    rows = rows.filter(e=>{
      ...
      const haystack = [
        ...
        e.lastEditedByName, e.downtimeReasonId ? nameOf(DB.reasons,e.downtimeReasonId) : '',
